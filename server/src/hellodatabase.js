...
MongoClient.connect(url, function(err, db) {
  if (err) {
    throw new Error("Could not connect to databse: " + err);
  } else {
 ...
 * operation completes. We will pass back the new object's ID.
 */
function insertExample(db, callback) {
  ...
  db.collection('helloworld').insertOne(exampleDocument, function(err, result) {
    if (err) {
      ...
      // Success!
      console.log("Successfully updated databse! The new object's ID is " + result.insertedID);
